...
def _fetch_queries(conn: sqlite3.Connection, group_id: int) -> List[str]:
    rows = conn.execute(
        "SELECT raw_query FROM group_queries WHERE group_id = ? ORDER BY position, id",
        (group_id,),
    ).fetchall()
...
def list_groups() -> List[Dict[str, Any]]:
    with connect() as conn:
        rows = conn.execute(
            "SELECT id, name, label, tags_json, context, created_at, updated_at FROM keyword_groups ORDER BY name"
        ).fetchall()
...
def get_group(name_or_id: str | int) -> Dict[str, Any]:
    ...
    with connect() as conn:
        row = conn.execute(
            f"SELECT id, name, label, tags_json, context, created_at, updated_at FROM keyword_groups WHERE {field} = ?",
...
def create_group(*, name: str, queries: List[str], label: str | None = None, tags: Iterable[str] | None = None, context: str | None = None) -> Dict[str, Any]:
    ...
    with connect() as conn:
        cur = conn.execute(
            "INSERT INTO keyword_groups(name, label, tags_json, context, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)",
            (name, label, json.dumps(tag_values, ensure_ascii=False), context, now, now),
        )
        group_id = int(cur.lastrowid)
        for idx, query in enumerate(clean_queries, start=1):
            conn.execute(
                "INSERT INTO group_queries(group_id, position, raw_query) VALUES (?, ?, ?)",
                (group_id, idx, query),
            )
...
def update_group(
    ...
    *,
    ...
    tags: Iterable[str] | None | object = None,
    replace_queries: List[str] | None = None,
    add_queries: List[str] | None = None,
    remove_queries: List[str] | None = None,
) -> Dict[str, Any]:
    ...
    if tags is None:
        ...
        new_tags = _normalize_tags(tags)  # type: ignore[arg-type]
    ...
    with connect() as conn:
        conn.execute(
            "UPDATE keyword_groups SET label = ?, tags_json = ?, context = ?, updated_at = ? WHERE id = ?",
            (new_label, json.dumps(new_tags, ensure_ascii=False), new_context, now, group_id),
        )
...
def remove_group(name_or_id: str | int) -> int:
    ...
    with connect() as conn:
        cur = conn.execute(
            f"DELETE FROM keyword_groups WHERE {field} = ?",
            (int(name_or_id) if field == "id" else name_or_id,),
        )
